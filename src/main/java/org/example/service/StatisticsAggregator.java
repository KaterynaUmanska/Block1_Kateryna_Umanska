...
public class StatisticsAggregator {
    ...
    private final Map<String, LongAdder> statistics =
    ...
    public StatisticsAggregator(String targetAttribute) {
        ...
        if (targetAttribute == null || targetAttribute.isBlank()) {
            throw new IllegalArgumentException(
                    "Атрибут для статистики не може бути порожнім"
            );
        }

        this.targetAttribute =
                targetAttribute.toLowerCase(Locale.ROOT);

        if (!SUPPORTED_ATTRIBUTES.contains(this.targetAttribute)) {
            throw new IllegalArgumentException(
                    "Непідтримуваний атрибут: "
                            + targetAttribute
            );
        }
    }
    ...
    public void process(PurchaseTransaction transaction) {
        ...
        switch (targetAttribute) {
            case "tags" -> processMultiValueAttribute(
                    transaction.getTags()
            );
            ...
            case "unit" -> {
                if (transaction.getMaterial() != null
                        && transaction.getMaterial().getUnit() != null) {
            ...
            case "material" -> {
                if (transaction.getMaterial() != null
                        && transaction.getMaterial().getName() != null) {
            ...
            default -> throw new IllegalStateException(
                    "Невідомий атрибут: " + targetAttribute
            );
        }
    }
